...
class MixtureJointDiBS(MixtureDiBS):
    ...
    def _svgd_step(self, t, c, opt_state_z, opt_state_theta, key, sf_baseline, E_k, E_k_stack):
        ...
        if self.parallell_computation:
            debug.print('Computing parameter gradient (parallell).')
            ...
            debug.print('Computing graph gradient (parallell).')
            ...
            debug.print('Computing parameter gradient (nonparallell).')
            ...
            debug.print('Computing graph gradient (nonparallell).')
            # d/dz log p(theta, D | z)
            key, *batch_subk = random.split(key, n_particles + 1)
