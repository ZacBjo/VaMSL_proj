...
class edgeElicitation:
    ...
    def nested_monte_carlo_EIG(self, key, parameter_samples, experiment):
        ...
        key, subkeys = key, *batch_subk = random.split(key, N + 1)
        simulate_response = lambda key, ps, exp: simulator.run_experiment(key=key, edge_probs=p, experiment=exp)
        ys = vmap(simulate_response, (0, 0, None))(jnp.array(subkeys), parameter_samples, experiment)
        
        # Compute likelihood of each draw p(y^(s)|Z_k^(s), exp_ij )
        log_y_liks = vmap(simulator.get_experiment_log_likelihood, (0, 0, None))(y, parameter_samples, experiment)
        
        # marginalize likelihood for each draw  
        # nu^(s) = 1/s' \sum_s' p(y^(s)|Z_k^(s'), exp_ij), s=1,...,S
        log_nu_s = lambda y, parameters, experiment: logsumexp(a=jnp.array([simulator.get_experiment_log_likelihood(y, parameter, experiment) for parameter in parameters]), b=(1/N))
    ...
    def experiment_EIG(self, parameter_samples, experiment):
        ...
        if self.expected_utility == 'Rao-Blackwellized EIG':
            ...
            EIG = nested_monte_carlo_EIG(parameter_samples, experiment)
    ...
    def optimal_queries(self, *, parameter_samples, experiment_list, n_queries=1, key=None):
        ...
        EIGs = vmap(self.experiment_EIG, (None, 0))(parameter_samples, experiment_list)
